...
- **The DAY_RANGE ceiling is discovered, not assumed.** FIRMS documents a
  1–10 day range but currently enforces 1–5. The client reads the real limit
  out of the rejection message, adopts it for the rest of the run, and
  re-splits the affected block, so an API-side change does not stop a run.
