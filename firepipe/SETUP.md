...
```bash
firepipe run -c up_punjab_haryana_mp.yaml --density
```
...
FIRMS enforces a 5-day maximum despite documenting 10. The client reads the real
ceiling from the rejection message and re-splits automatically.

**`ArrowInvalid: Could not convert ... with type str`**
Fixed in v1.0.2. `confidence` and `version` are always text, with typed
`confidence_class` / `confidence_value` columns beside them. CSV is written
before parquet, so a serialisation fault cannot discard a completed run.

**Masking is slow on the first run**
Expect roughly 25 minutes for 400k detections over /vsicurl. Sampled values are
cached by coordinate under `~/.cache/firepipe/masks`, so later runs take
seconds. Set `mask.cache_samples: false` to disable.
